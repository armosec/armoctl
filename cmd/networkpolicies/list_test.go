...
func TestList_PostsList(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if !strings.HasSuffix(r.URL.Path, "/networkpolicies") {
			t.Errorf("path: got %s, want suffix /networkpolicies", r.URL.Path)
		}
		...
		_ = json.NewEncoder(w).Encode(map[string]any{
			"response": []map[string]any{{"name": "deny-all", "namespace": "default", "cluster": "prod", "kind": "NetworkPolicy"}},
	...
	if !strings.Contains(stdout.String(), "deny-all") {
