...
func TestECSList_PostsList(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.Method != http.MethodPost {
			t.Errorf("method: got %s, want %s", r.Method, http.MethodPost)
		}
		if !strings.HasSuffix(r.URL.Path, "/accounts/ecs/list") {
			t.Errorf("path: got %s, want suffix /accounts/ecs/list", r.URL.Path)
		}
		...
		_ = json.NewEncoder(w).Encode(map[string]any{
			"response": []map[string]any{{"clusterARN": "arn:aws:ecs:us-east-1:123456789:cluster/prod", "name": "prod", "region": "us-east-1", "accountID": "123456789", "status": "connected"}},
	...
	root.AddCommand(ECSListCmd(func(cmd *cobra.Command) *apiclient.Client { return c }))
