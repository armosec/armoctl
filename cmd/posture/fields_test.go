...
func TestCheatsheetCoversAllScopes(t *testing.T) {
	cs := Cheatsheet()
	for _, want := range []string{"frameworks", "controls", "resources", "exceptions"} {
		if len(cs[want]) < 4 {
...
func TestFieldsCmd_OneScope(t *testing.T) {
	...
	var buf bytes.Buffer
	cmd.SetOut(&buf)
	cmd.SetArgs([]string{"frameworks"})
	if err := cmd.Execute(); err != nil {
		t.Fatal(err)
	}
	if !strings.Contains(buf.String(), "complianceScore") {
		t.Fatalf("output missing complianceScore: %s", buf.String())
	}
}
