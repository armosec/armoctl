...
func TestCreate_DryRunDoesNotCallServer(t *testing.T) {
	...
	root.SetOut(&stdout)
	root.SetArgs([]string{"runtime-policies", "create", "--name", "test-policy", "--dry-run"})
...
func TestCreate_YesPostsAndReports(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if body["enabled"] != true {
			t.Errorf("body.enabled: %v", body["enabled"])
		}
		w.Header().Set("x-request-id", "req-z")
		w.WriteHeader(200)
		_, _ = w.Write([]byte(`{"guid":"p1"}`))
	}))
	defer srv.Close()
	c := apiclient.New(apiclient.Config{BaseURL: srv.URL, AccessKey: "K", CustomerGUID: "G"})
	...
	var stdout bytes.Buffer
	root.SetOut(&stdout)
	root.SetArgs([]string{"runtime-policies", "create", "--name", "test-policy", "--yes"})
	if err := root.ExecuteContext(context.Background()); err != nil {
		t.Fatal(err)
	}
...
func TestUpdate_DryRunShowsPUT(t *testing.T) {
	...
	var stdout bytes.Buffer
	root.SetOut(&stdout)
	root.SetArgs([]string{"runtime-policies", "update", "p1", "--name", "updated", "--dry-run"})
	if err := root.ExecuteContext(context.Background()); err != nil {
		t.Fatal(err)
	}
	if atomic.LoadInt32(&hits) != 0 {
	...
	if req["url"] != "/runtime/policies/p1" {
		t.Fatalf("expected /runtime/policies/p1 path: %v", req)
	}
