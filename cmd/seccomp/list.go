...
func ListCmd(clientFor cliclient.ClientFor) *cobra.Command {
	c := &cobra.Command{
		Use:   "list",
		Short: "List seccomp profiles",
		RunE: func(cmd *cobra.Command, args []string) error {
			cli := clientFor(cmd)
			pg := cliflags.ReadPage(cmd)
			body := map[string]any{}
			res, err := cli.ListPaged(cmd.Context(), "/seccomp/list", nil, apiclient.ListOpts{
				Method:   "POST",
				Body:     body,
				Limit:    pg.Limit,
				Page:     pg.Page,
			...
			return output.Render(cmd.OutOrStdout(), list, cliflags.OutputOptions(cmd, ProfileSummary))
		},
	}
	return c
}
